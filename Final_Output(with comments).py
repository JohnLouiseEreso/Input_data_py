...
def show_data():
    ...
    def delete_selected_date():
        #Deletes all expense records for a selected date from the database and updates the UI.

        # Get the selected item from the treeview
        selected_item = tree.focus()

        # If no date is selected, show a warning message and exit the function
        if not selected_item:
            messagebox.showwarning("Warning", "Please select a date to delete.")
            return

        # Retrieve values associated with the selected item
        values = tree.item(selected_item, "values")

        # If there are no values, exit the function
        if not values:
            return

        selected_date = values[0]  # Extract the selected date

        # Ask for user confirmation before deleting records
        confirm = messagebox.askyesno("Confirm Deletion", f"Are you sure you want to delete all expenses for {selected_date}?")

        if confirm:  # Proceed only if the user confirms deletion
            conn = sqlite3.connect("input.db")  # Connect to the SQLite database
            cursor = conn.cursor()  # Create a cursor object

            # Execute a DELETE query to remove all expenses for the selected date
            cursor.execute("DELETE FROM expenses WHERE date = ?", (selected_date,))
            conn.commit()  # Commit changes to the database
            conn.close()  # Close the connection

            # Remove the selected date from the treeview to reflect the deletion in UI
            tree.delete(selected_item)

            print(f"Deleted all expenses for {selected_date}.")  # Log deletion for debugging
            messagebox.showinfo("Deleted", f"All expenses for {selected_date} have been deleted.")  # Show confirmation message


    # Function to display pie chart inside details window
    def show_pie_chart(window, selected_date):
        # Connect to the database and fetch expense data grouped by category
        conn = sqlite3.connect("input.db")  
        cursor = conn.cursor()
        
        # Query to get the sum of expenses for each category on the selected date
        cursor.execute("SELECT category, SUM(amount) FROM expenses WHERE date = ? GROUP BY category", (selected_date,))
        data = cursor.fetchall()  
        conn.close()  # Close the database connection

        # If no expense data is found for the selected date, exit the function
        if not data:
            return  

        # Extract category labels and their corresponding total expenses
        labels = [str(row[0]) for row in data]  # Extract category names
        sizes = [row[1] for row in data]  # Extract sum of amounts per category

        # Create a new figure for the pie chart with a fixed size
        ...
        ax.set_title(f"Expense Distribution on {selected_date}")  # Set title

        # Embed the pie chart in the Tkinter window
        canvas = FigureCanvasTkAgg(fig, master=window)  
        canvas.draw()  
        canvas.get_tk_widget().pack(pady=10)  # Display the pie chart widget
...
def create_expense_tracker_gui():
    """Creates the main GUI for the expense tracker application, including input fields, buttons, and a table display."""

    global date_entry, category_combo, item_entry, amount_entry, tree
    global day_label, total_label, remaining_label, root, remaining_budget

    # Set window size
    root.geometry("880x450")

    # Initialize remaining budget from user input
    remaining_budget = float(budget_entry.get())  # Convert user input to float

    # Configure Treeview style (table headers)
    style = ttk.Style()
    style.configure("Treeview.Heading", font=("Arial", 12, "bold"))

    # Day Label to display the current day of tracking
    ...
    category_combo = ttk.Combobox(form_frame, font=("Arial", 12), width=18, 
                                  values=["Food", "Transport", "Entertainment", "Rent", "Utilities", "Others"])
    ...
    # Pack the table inside the frame
    tree.pack(expand=True, fill="both")

    # Footer Section for Remaining Budget and Total Spent
...
def reset_program():
    ...
    # Reconfigure the root window size and background color
    root.geometry("400x500")
    root.configure(bg="purple")
    
    # Create a new welcome screen frame
    ...
    # Button to start the budgeting process
    ...
    # Button to show the saved expense list
    save_button = tk.Button(welcome_screen, text="Show List", command=show_data, font=("Arial", 16, "bold"), 
                            fg="white", bg="purple")
...
def display_table_Chart(data, budget):  
    """Displays the summary table and pie chart after completing the budgeting duration."""
    global chart_frame
    ...
    # Table frame for displaying expenses
    table_frame = tk.Frame(content_frame)
    table_frame.pack(side="top", fill="both", expand=True, pady=(0, 20))

    # Set the height of the table (max 50 rows)
    ...
    # Retrieve the remaining budget
    budget_left = budget[0]

    # Summary row showing remaining budget and total expenses
    ...
    back_button = tk.Button(button_frame, text="Back to Main", 
                            command=reset_to_main,  # Calls the reset function
                            bg="blue", fg="white")  
...
def main():
    ...
    # Start Journey Button - Navigates to budget setup screen
    start_button = tk.Button(welcome_screen, text="START YOUR JOURNEY!", font=("Arial", 12, "bold"),
                             bg="lightgray", command=go_to_budget_screen)
    start_button.pack(pady=20)

    # Show Data Button - Opens the saved data list
    ...
    title_entry.insert(0, "Title")  # Placeholder text
    title_entry.bind("<FocusIn>", clear_placeholder)  # Clear placeholder when focused
    title_entry.bind("<FocusOut>", restore_placeholder)  # Restore placeholder when unfocused
    title_entry.pack(pady=5)

    # Duration Entry Field
    duration_entry = tk.Entry(budget_screen, width=30, font=("Arial", 12), fg="grey")
    duration_entry.insert(0, "Enter Duration")  # Placeholder text
